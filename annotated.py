...
def decorate(win):
    ...
    # the decorations in response to UnmapNotify events, which the reparenting
    # generates, we'd better ignore the UnmapNotify generated by reparenting,
...
while 1:
    ...
    # A window was reparented, we can stop ignoring unmaps for it now
    ...
    # A window was unmapped
        ...
        frame, win = windows.get(ev.window, (None, None))
        # It's a decorated window, and the unmap did not result from reparenting,
        # destroy the decorations.
        if ev.window == win and win not in reparenting:
            # Reparent the window to the root so that it's not destroyed when
            # we destroy the frame.
            win.reparent(root, 0, 0)
            # Remove the window from our save set so that it's not automatically
            # mapped if we die.
            win.change_save_set(X.SetModeDelete)
            # Remove association data, destroy decorations
            del windows[frame]
            del windows[win]
            frame.destroy()
